...
try {
    switch ($action) {
        ...
        case 'agent-payments':
            // GET /payment-api.php/agent-payments?status=pending
            $user = requireAuthUser(['agent']);
            
            $agentId = $user['user_id'] ?? null;
            $status = $_GET['status'] ?? 'all'; // all, pending, paid, overdue
            $clientId = $_GET['client_id'] ?? null;
            
            $result = $paymentService->getAgentClientPayments($agentId, $status, $clientId);
